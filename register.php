...
if ($stmt = $con->prepare('SELECT id, password FROM accounts WHERE username = ?')) {
	...
	if ($stmt->num_rows > 0) {
		...
		echo 'Username exists, please choose another!';
            ...
            if ($stmt = $con->prepare('INSERT INTO accounts (username, password, email, admin, membershiptype) VALUES (?, ?, ?, ?, ?)')) {
				...
				$stmt->bind_param('ssss', $_POST['username'], $password, $_POST['email'], $admin, $membershipType);
